...
parser.add_argument('--resume', default='checkpoint/PU_basis/ppunet_epoch_300.pth', type=str)
...
if __name__ == '__main__':
    model = PPUNet(npoint=2048, up_ratio=args.up_ratio, use_normal=False, use_bn=args.use_bn, use_res=args.use_res)
    ...
    for itr, batch in enumerate(test_loader):
        ...
        pred_skeleton = pred_skeleton.data.cpu().numpy()
        gt_skeleton = gt_skeleton.data.cpu().numpy()

        save_ply(os.path.join(args.save_dir, '{}_input.ply'.format(itr)), input_data[0, :, :3])
        save_ply(os.path.join(args.save_dir, '{}_input_sk.ply'.format(itr)), gt_skeleton[0, :, :3])
        ...
        save_ply(os.path.join(args.save_dir, '{}_output_skeleton.ply'.format(itr)), pred_skeleton[0])

        f=open(os.path.join(args.save_dir,'test_loss.txt'),'a')
        f.write('\n'+' loss {:.4f}, weighted emd loss {:.4f}, cd loss{:.4f}, repul loss{:.4f}, iter{}'.format(float(loss), float(emd_loss),float(cd_loss),float(repulsion_loss),int(itr)))
        f.close()

    print(' loss {:.4f}, weighted emd loss {:.4f}, cd loss {:.4f}, repul loss{:.4f}.'.format(np.mean(loss_list), \
    np.mean(emd_loss_list), np.mean(cd_loss_list), np.mean(repu_loss_list)))
